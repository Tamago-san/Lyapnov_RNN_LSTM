...
!
!モデルのタイムスケールの0.01で進む。
!gfortran LSTM_LY.f90 lstm_tanh.f90 -o LSTM_LY.out -llapack -lblas
!-----------------------------------------------------------------------------
module lyapnov
    implicit none
...
!    real(8), parameter :: par_c = 4.d0
    ...
    real(8), parameter :: epsi0 =1.d-4
    real(8) w_out(out_node,rc_node)
    real(8) w_in (in_node,rc_node,4)
    real(8) W_rnn(rc_node,rc_node,4)
    real(8) W_cel(rc_node,4)
  contains
    subroutine march(R,Z,C,dc,U_rc,S_rc,abs_dr,fstep,ftau,gusai,alpha,G,allstep)
        real(8) U_rc(:,:)
        real(8) S_rc(:,:)
        real(8) R(0:tau,1:rc_node)
        real(8) dc(0:tau,1:rc_node)
        real(8) Z(0:tau,1:rc_node,4)
        real(8) C(0:tau,1:rc_node)
        real(8) wu1(rc_node),wu2(rc_node),wu3(rc_node),wu4(rc_node)
        real(8) wr1(rc_node),wr2(rc_node),wr3(rc_node),wr4(rc_node)
        
        real(8) DF(rc_node, rc_node)
        real(8) gusai,alpha,G,abs_dr
        integer i,j,k,f,fstep,ftau,step,allstep,f1,f2
!        write(*,*) dr_befor(1:par_node)
        ...
        do f1=1,rc_node
                do f2=1,in_node
                    wu1(f1) = wu1(f1) + W_in(f1,f2,1) * U_rc(step,f2)
                    wu2(f1) = wu2(f1) + W_in(f1,f2,2) * U_rc(step,f2)
                    wu3(f1) = wu3(f1) + W_in(f1,f2,3) * U_rc(step,f2)
                    wu4(f1) = wu4(f1) + W_in(f1,f2,4) * U_rc(step,f2)
                enddo
                do f2=1,rc_node
                    wr1(f1) = wr1(f1) + W_rnn(f1,f2,1) *R(ftau-1,f2)
                    wr2(f1) = wr2(f1) + W_rnn(f1,f2,2) *R(ftau-1,f2)
                    wr3(f1) = wr3(f1) + W_rnn(f1,f2,3) *R(ftau-1,f2)
                    wr4(f1) = wr4(f1) + W_rnn(f1,f2,4) *R(ftau-1,f2)
                enddo
                z(ftau,f1,1)=wu1(f1) + wr1(f1) + W_cel(f1,1)*c(ftau-1,f1)
                z(ftau,f1,2)=wu2(f1) + wr2(f1) + W_cel(f1,2)*c(ftau-1,f1)
                z(ftau,f1,3)=wu3(f1) + wr3(f1)
                
                C(ftau,f1)=sigmoid( z(ftau,f1,1) ) * C(ftau-1,f1)&
                                + sigmoid(z(ftau,f1,2)) * tanh(z(ftau,f1,3))
                
                z(ftau,f1,4)=wu4(f1) + wr4(f1) + W_cel(f1,4)*c(ftau,f1)

                R(ftau,f1) = tanh(C(ftau,f1))*sigmoid(z(ftau,f1,4))
        enddo
        do i = 1,out_node
		    s_rc(ftau,i)  = 0.d0
		    do k = 1,rc_node
        ...
        do j=1,rc_node
        do i=1,rc_node
            DF(j,i)=d_sigmoid(z(ftau,j,1))*W_cel(i,1)*c(ftau,j) + sigmoid(z(ftau,j,2))*tanh(z(ftau,j,3))&
                    +sigmoid(z(ftau,j,1))*c(ftau,j)+ d_sigmoid(z(ftau,j,2))*W_cel(i,2)*tanh(z(ftau,j,3))
            if (i==j) then
                DF(j,i) = DF(j,i) +sigmoid(z(ftau,j,1)) + sigmoid(z(ftau,j,2))*tanh(z(ftau,j,3))
            endif
        enddo
        enddo
        ...
        do i=1,rc_node
            dc(ftau,i) = 0.d0
            do k=1,rc_node
!                write(*,*) ftau, dc(ftau-1,k),DF(k,i),dc(ftau-1,k)*DF(k,i)
                dc(ftau,i) = dc(ftau,i) + dc(ftau-1,k)*DF(k,i)
!            write(*,*) dr(ftau,i)
            enddo
        enddo
...
!                    u_tr,s_tr,U_rc,w_in,w_rnn,w_out)
    ...
    call lstm_traning_own_fortran(in_node,out_node,rc_node,traning_step,rc_step,&
                    sample_num,epoch,iepsi,g0,gusai0,&
                    u_tr,s_tr,u_rc,w_out,w_rnn,w_cel,w_in)
...
!        write(*,*) dr(1:3)
        do itau = 1,tau
            ...
            call march(R,Z,C,dr,U_rc,S_rc_out,abs_dr,istep,itau,gusai0,1.d0,G0,rc_step)
